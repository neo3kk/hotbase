...
// Función asíncrona que devuelve la promesa de los datos
...
export default function CarDetailsView({ params }: { params: Promise<{ id: string }> }) {
  // 1. Desenvuelve la promesa `params` con React.use()
  const resolvedParams = React.use(params);

  // 2. Desenvuelve la promesa de los datos del coche con React.use()
  const car = React.use(fetchCarDetails(resolvedParams.id));

  if (!car) notFound();

  // El JSX para renderizar los detalles
  return (
          ...
          {car.car_images && car.car_images.length > 0 ? (
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-3 gap-4">
                {car.car_images.slice(1).map((image: { id: string; image_url: string }) => (
        ...
        <div className="space-y-6">
          ...
          {car.notes && (
            <div className="space-y-1">
              <p className="text-gray-500">Notas</p><p className="whitespace-pre-wrap">{car.notes}</p>
            </div>
          )}
        </div>
      </div>
